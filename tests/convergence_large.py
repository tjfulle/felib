...
def run_case(
    element_obj, nelx, nely, traction_mag=200.0, youngs_modulus=1.0e03, poissons_ratio=0.25
):
    mesh, _, _, _, _ = build_cantilever_mesh(nelx=nelx, nely=nely)

    model = felib.model.Model(
        mesh, name=f"cantilever_{element_obj.__class__.__name__}_{nelx}x{nely}"
    )
